...
static std::string normalize_nfd(const std::string &s) {
	std::string ret;
	char *result = (char *)duckdb::utf8proc_NFD((unsigned char *)s.c_str(), s.length());
	if (result) {
...
static std::wstring convertToUnicode(const std::string &text) {
	...
	while (i < text.size()) {
		...
		utf8proc_ssize_t forward =
		    duckdb::utf8proc_iterate((utf8proc_uint8_t *)&text[i], text.size() - i, (utf8proc_int32_t *)&codepoint);
		if (forward < 0)
			return L"";
		ret += codepoint;
		i += forward;
...
std::string convertFromUnicode(const std::wstring &wText) {
	...
	std::string ret;
	for (auto ch : wText) {
		utf8proc_ssize_t num = duckdb::utf8proc_encode_char(ch, (utf8proc_uint8_t *)dst);
		if (num <= 0)
			return "";
...
static std::wstring tolower(const std::wstring &s) {
	...
	for (size_t i = 0; i < s.size(); i++) {
		ret[i] = duckdb::utf8proc_tolower(s[i]);
	}
	return ret;
}
...
bool BasicTokenizer::isControol(const wchar_t &ch) const {
	if (ch == L'\t' || ch == L'\n' || ch == L'\r')
		return false;
	auto cat = duckdb::utf8proc_category(ch);
	if (cat == duckdb::UTF8PROC_CATEGORY_CC || cat == duckdb::UTF8PROC_CATEGORY_CF)
		return true;
	return false;
}
...
bool BasicTokenizer::isWhitespace(const wchar_t &ch) const {
	if (ch == L' ' || ch == L'\t' || ch == L'\n' || ch == L'\r')
		return true;
	auto cat = duckdb::utf8proc_category(ch);
	if (cat == duckdb::UTF8PROC_CATEGORY_ZS)
		return true;
	return false;
}
...
bool BasicTokenizer::isPunctuation(const wchar_t &ch) const {
	if ((ch >= 33 && ch <= 47) || (ch >= 58 && ch <= 64) || (ch >= 91 && ch <= 96) || (ch >= 123 && ch <= 126))
		return true;
	auto cat = duckdb::utf8proc_category(ch);
	if (cat == duckdb::UTF8PROC_CATEGORY_PD || cat == duckdb::UTF8PROC_CATEGORY_PS ||
	    cat == duckdb::UTF8PROC_CATEGORY_PE || cat == duckdb::UTF8PROC_CATEGORY_PC ||
	    cat == duckdb::UTF8PROC_CATEGORY_PO // sometimes ¶ belong SO
	    || cat == duckdb::UTF8PROC_CATEGORY_PI || cat == duckdb::UTF8PROC_CATEGORY_PF)
		return true;
	return false;
}
...
std::wstring BasicTokenizer::runStripAccents(const std::wstring &text) const {
	...
	for (auto &ch : nText) {
		auto cat = duckdb::utf8proc_category(ch);
		if (cat == duckdb::UTF8PROC_CATEGORY_MN)
			continue;
		output += ch;
	}
	return output;
...
int FullTokenizer::tokenizeToIds(const std::string &text, int64_t *input_ids, int64_t *mask, int size,
                                 int start) const {
	int i = start;
	for (auto &token : mBasicTokenizer.tokenize(text)) {
		for (auto &subToken : mWordpieceTokenizer.tokenize(token)) {
